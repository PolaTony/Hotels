...
Public Class Frm_StockIssue_Invoices
    ...
    Private Sub sQuerySummaryMain(Optional ByVal pCode As String = "", Optional ByVal pDesc As String = "")
        Try
            ...
            If pCode = "" Then
                vCodeFilter = ""
            Else
                vCodeFilter = " And Stock_Issue.Code Like '%" & pCode & "%'"
            End If

            If pDesc = "" Then
                vDescFilter = ""
            Else
                vDescFilter = " And Stock_Issue.DescA Like '%" & pDesc & "%'"
            End If
            ...
            "  Select Stock_Issue.Code,                                        " &
            "         Stock_Issue.DescA as SE_DescA,                           " &
            "         HR1.DescA as Emp_DescA,                                  " &
            "         Stock_Issue.Remarks,                                     " &
            "         Stock_Issue.TDate                                        " &
            " From Stock_Issue Inner Join Employees   HR1                      " &
            " On Stock_Issue.Emp_Code = HR1.Code                               " &
            " Where 1 = 1                                                      " &
            " And Status = 'P'                                                 " &
            vCodeFilter & vbCrLf &
            vDescFilter & vbCrLf &
    ...
    Private Sub sQuerySummaryDetails(ByVal pRow As UltraDataRow, ByVal pChildBand As UltraDataBand)
        Try
            ...
            Dim vRowCounter As Integer
            vsqlCommand.CommandText =
            " Select Stock_Issue_Details.Ser,          " &
            "        Items.UserCode,                    " &
            "        Items.DescA as Item_Desc,              " &
            "        IsNull(Stock_Issue_Details.PU_Ser, 0),  " &
            "        Items_PackUnit.Number,                 " &
            "        Pack_Unit.DescA,                       " &
            "        Stock_Issue_Details.Quantity,     " &
            "        Stock_Issue_Details.Str_Code,     " &
            "        Stores.DescA as Str_Desc,              " &
            "        Stock_Issue_Details.Remarks        " &
            " From   Stock_Issue_Details Inner Join Items " &
            " On     Stock_Issue_Details.Item_Code = Items.Code " &
            "        Inner Join Stores                         " &
            " On     Stock_Issue_Details.Str_Code = Stores.Code " &
            "        Inner Join Pack_Unit                      " &
            " On     Pack_Unit.Code = Items.PU_Code " &
            "        Left Join Items_PackUnit                     " &
            " On     Stock_Issue_Details.Item_Code = Items_PackUnit.Item_Code       " &
            " And    Stock_Issue_Details.PU_Ser    = Items_PackUnit.PU_Code             " &
            " Where  SS_Code = '" & pRow("Code") & "'" &
            " Order  By Ser       "
    ...
    Handles Txt_FndByCode.ValueChanged, Txt_FndByDesc.ValueChanged,

...
#Region " Query                                                                                  "
    Public Sub sQuery(Optional ByVal pRecPos As Integer = Nothing, Optional ByVal pItemCode As String = Nothing, Optional ByVal pIsGoTo As Boolean = False)
        ...
        'vMasterBlock = "N"
    End Sub
    Private Sub sLoad_DeductionsTypes()
        Try
            Dim vRowCounter As Integer
            Dim vsqlCommand As New SqlCommand
            vsqlCommand.CommandText =
            " Select Code, Desc" & vLang & " From DeductionsTypes " &
            " Where 1 = 1 And Company_Code = " & vCompanyCode

            vsqlCommand.Connection = cControls.vSqlConn
            cControls.vSqlConn.Open()
            Dim vSqlReader As SqlDataReader = vsqlCommand.ExecuteReader
            Txt_DeductionsTypes.Items.Clear()

            Do While vSqlReader.Read
                Txt_DeductionsTypes.Items.Add(vSqlReader(0), vSqlReader(1))
            Loop
            cControls.vSqlConn.Close()
            Txt_DeductionsTypes.SelectedIndex = -1
...
#Region " Save                                                                                  "
    Private Function fValidateDetails() As Boolean
        ...
        For Each vRow In GRD_Details.Rows
            ...
            If IsDBNull(vRow.Cells("Deduction_Type").Value) Then
                vcFrmLevel.vParentFrm.sForwardMessage("170", Me)
                vRow.Cells("Deduction_Type").Selected = True
                Return False
            End If
        Next
...
#Region " Form Level                                                                            "
    ...
    Private Sub Grd_Additions_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles GRD_Details.KeyUp
        ...
        End Try
    End Sub
    Private Sub GRD_Additions_AfterRowInsert(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.RowEventArgs) Handles GRD_Details.AfterRowInsert
        Try
    ...
    Private Sub GRD_Details_KeyPress(sender As Object, e As KeyPressEventArgs) Handles GRD_Details.KeyPress
        If GRD_Details.ActiveCell IsNot Nothing AndAlso GRD_Details.ActiveCell.Column.Key = "TValue" Then
            ...
            If e.KeyChar = "." AndAlso GRD_Details.ActiveCell.Text.Contains(".") Then
                e.Handled = True
            End If
        End If
    ...
    Private Sub sQuerySummary()
        ...
        Try
            ...
            Do While vSqlReader.Read
                ...
                'TDate
                If IsDBNull(vSqlReader("TDate")) = False Then
                    DTS_Summary.Rows(vRowCounter)("TDate_Day") = vSqlReader("TDate")
                Else
                    DTS_Summary.Rows(vRowCounter)("TDate_Day") = Nothing
                End If
                ...
                'Ded_Type
                If IsDBNull(vSqlReader("Ded_Type")) = False Then
                    DTS_Summary.Rows(vRowCounter)("Ded_Type") = vSqlReader("Ded_Type")
                Else
                    DTS_Summary.Rows(vRowCounter)("Ded_Type") = Nothing
                End If
...
#End Region
    Private Sub sImportFromExcel()
        Try
            ...
            Dim result As String = $"Variable is {x}"

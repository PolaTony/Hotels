...
Public Class Frm_Performance_A
#Region " Declaration                                                                           "
...
#Region " My Form                                                                               "
    ...
    Private Sub FRM_AddDed_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        ...
        sQuerySummary()
    End Sub
    Private Sub Txt_All_ValueChanged(sender As Object, e As EventArgs) Handles _
        Txt_EmpDesc.ValueChanged, Txt_TDate.ValueChanged
...
#Region " New Record                                                                            "
    ...
    Private Sub sNewCode()
        ...
        vSqlString = " Select IsNull(Max(Convert(Int, Code)), 0) + 1 From  Performance " &
                     " Where Company_Code = " & vCompanyCode

        Txt_Code.Text = cControls.fReturnValue(vSqlString, Me.Name)

    End Sub
    Private Sub sLoad_PerformanceTypes()
        Try
            ...
            Do While vSqlReader.Read
                ...
                DTS_Details.Rows(vRowCounter)("Performance_Code") = Trim(vSqlReader(0))

                If vSqlReader.IsDBNull(1) = False Then
                    DTS_Details.Rows(vRowCounter)("Performance_Desc") = Trim(vSqlReader(1))
                End If
...
#Region " Master                                                                                "
    Private Function fValidateMaster() As Boolean
        ...
        If Txt_TDate.Value Is Nothing Then
    ...
    Private Sub sSaveMaster()
        ...
        vDate = " '" & Strings.Format(Txt_TDate.Value, "MM-dd-yyyy") & "' "

        If vMasterBlock = "I" Then
            sNewCode()

            vSqlString = " Insert Into Performance (         Code,              Company_Code,         TDate,           Emp_Code,              User_Code    ) " &
                         "                  Values ( " & Txt_Code.Text & ", " & vCompanyCode & ", " & vDate & ", " & Txt_EmpCode.Text & ", " & vUsrCode & ") "
    ...
    Private Sub TXT_All_EditorButtonClick(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinEditors.EditorButtonEventArgs) _
        Handles Txt_EmpCode.EditorButtonClick
    ...
    Private Sub sQueryDetails()
        Try
            ...
            Do While vSqlReader.Read
                ...
                vRow = GRD_Details.Rows(vRowCounter)
                vRow.Activation = Activation.AllowEdit

                'Ser
                DTS_Details.Rows(vRowCounter)("Ser") = vSqlReader(0)

                'Emp_Code
                If vSqlReader.IsDBNull(1) = False Then
                    DTS_Details.Rows(vRowCounter)("Emp_Code") = vSqlReader(1)
                Else
                    DTS_Details.Rows(vRowCounter)("Emp_Code") = Nothing
                End If

                'TDate
                If vSqlReader.IsDBNull(2) = False Then
                    DTS_Details.Rows(vRowCounter)("TDate") = vSqlReader(2)
                Else
                ...
                If vSqlReader.IsDBNull(3) = False Then
                    DTS_Details.Rows(vRowCounter)("SignIn") = CDate(vSqlReader(3))
                Else
                    DTS_Details.Rows(vRowCounter)("SignIn") = Nothing
                End If

                'SignOut
                If vSqlReader.IsDBNull(4) = False Then
                    DTS_Details.Rows(vRowCounter)("SignOut") = vSqlReader(4)
                Else
                    DTS_Details.Rows(vRowCounter)("SignOut") = Nothing
                End If

                'Net Hours
                If vSqlReader.IsDBNull(5) = False Then
                    DTS_Details.Rows(vRowCounter)("NetHours") = vSqlReader(5)
                Else
                    DTS_Details.Rows(vRowCounter)("NetHours") = Nothing
                End If

                'Over Time
                If vSqlReader.IsDBNull(6) = False Then
                    DTS_Details.Rows(vRowCounter)("OverTime") = vSqlReader(6)
                Else
                    DTS_Details.Rows(vRowCounter)("OverTime") = Nothing
                End If

                'Remarks
                If vSqlReader.IsDBNull(7) = False Then
                    DTS_Details.Rows(vRowCounter)("Remarks") = vSqlReader(7)
                Else
                    DTS_Details.Rows(vRowCounter)("Remarks") = Nothing
                End If

                'Approved
                If vSqlReader.IsDBNull(8) = False Then
                    DTS_Details.Rows(vRowCounter)("Approved") = vSqlReader(8)
                Else
                    DTS_Details.Rows(vRowCounter)("Approved") = False
                End If
    ...
    Private Sub sSaveDetails()
        Try
            ...
            For Each vRow In GRD_Details.Rows
                ...
                If vRow.Cells("DML").Value = "I" Then
                    vSqlString = " Insert Into Performance_Details (      Prf_Code,              Company_Code,                      PrfType_Code,                               TValue,                 User_Code,                      Remarks ) " &
                                 "                          Values ( " & Txt_Code.Text & ", " & vCompanyCode & ", " & vRow.Cells("Performance_Code").Value & ", " & vRow.Cells("TValue").Value & " , " & vUsrCode & ", '" & vRow.Cells("Remarks").Text & "' )"
...
#End Region
#Region " Form Level                                                                            "
    Private Sub GRD_Addtransaction_BeforeCellDeactivate(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs)
    ...
    Private Sub GRD_AddTransaction_CellChange(ByVal sender As System.Object, ByVal e As Infragistics.Win.UltraWinGrid.CellEventArgs) Handles GRD_Details.CellChange
        ...
        End If
    End Sub
    Private Sub Grd_Additions_KeyUp(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        Try
            GRD_Details.UpdateData()
        ...
        End Try
    End Sub
    Private Sub GRD_Additions_AfterRowInsert(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.RowEventArgs)
        Try
            If e.Row.Index <> 0 Then
                If Not IsDBNull(GRD_Details.Rows(e.Row.Index - 1).Cells("TDate").Value) Then
                    e.Row.Cells("TDate").Value = GRD_Details.Rows(e.Row.Index - 1).Cells("TDate").Value
                End If
            End If

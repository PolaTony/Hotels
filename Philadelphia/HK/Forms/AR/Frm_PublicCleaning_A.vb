...
#Region "My Form"
    ...
    Private Sub Txt_All_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) _
        Handles Txt_CleaningType.ValueChanged, Txt_Supervisor.ValueChanged,
                Txt_CleaningDate.ValueChanged, Txt_Remarks.ValueChanged, Txt_PublicCleaning.ValueChanged
...
#Region " Query                                                                          "
    Public Sub sQuery(Optional ByVal pRecPos As Integer = Nothing, Optional ByVal pItemCode As String = Nothing, Optional ByVal pIsGoTo As Boolean = False)
        ...
        If pItemCode = Nothing Then
            If pIsGoTo = False Then
                ...
                Else
                    ...
                    If vFetchRec > cControls.fCount_Rec(" From  HK_Cleaning_Public " &
        ...
        End If
        If pRecPos = -2 Then
            vFetchRec = cControls.fCount_Rec(" From HK_Cleaning_Public " &
        ...
        Else
            vFetchCondition = " And MyPublicCleaning.Code = '" & Trim(pItemCode) & "'"
        End If

        Try
            Dim vSQlcommand As New SqlCommand
            vSQlcommand.CommandText =
            " With MyPublicCleaning as                                            " &
            "( Select HK_Cleaning_Public.Code,                                       " &
            "         HK_Cleaning_Public.Public_Code,                                  " &
            "         HK_Cleaning_Public.CleanType_Code,                             " &
            "         HK_Cleaning_Public.Supervisor_Code,                            " &
            "         HK_Cleaning_Public.Remarks,                                    " &
            "         HK_Cleaning_Public.Company_Code,                               " &
            "         ROW_Number() Over (Order By HK_Cleaning_Public.Code) as RecPos " &
            "                                                                     " &
            " From HK_Cleaning_Public                                                " &
            "                                                                     " &
            " LEFT JOIN HK_Public_Types                                     " &
            " ON HK_Cleaning_Public.Public_Code = HK_Public_Types.Code         " &
            " And HK_Cleaning_Public.Company_Code = HK_Public_Types.Company_Code )  " &
            "                                                                     " &
            " Select * From MyPublicCleaning                                      " &
            " Where  1 = 1                                                        " &
            " And    MyPublicCleaning.Company_Code = " & vCompanyCode &
            vFetchCondition
            ...
            Do While vSqlReader.Read
                ...
                'Public_Code
                If IsDBNull(vSqlReader("Public_Code")) = False Then
                    Txt_PublicCleaning.Value = vSqlReader("Public_Code")
                Else
                    Txt_PublicCleaning.SelectedIndex = -1
                End If
    ...
    Public Sub sDelete()
        ...
        ElseIf vMasterBlock = "N" Or vMasterBlock = "U" Then
            If vcFrmLevel.vParentFrm.sForwardMessage("39", Me) = MsgBoxResult.Yes Then
                ...
                vSqlstring =
                " Delete From HK_Cleaning_Public_Details " &
                " Where  CL_Code = '" & Txt_Code.Text & "' " &
                " And    Company_Code = " & vCompanyCode &
                "                                           " &
                " Delete From HK_Cleaning_Public " &
...
#Region " New Record                                                                     "
    Public Sub sNewRecord()
        ...
        Txt_PublicCleaning.Select()

        DTS_Details.Rows.Clear()
    End Sub
    Private Sub sNewCode()
        Dim vSqlString As String
        vSqlString = " Select IsNull(Max(Convert(Int, Code)), 0) + 1 From HK_Cleaning_Public " &
                     " Where  Company_Code = " & vCompanyCode
...
#Region " Save                                                                           "
    Private Function fValidateMain() As Boolean
        ...
        If Txt_PublicCleaning.Text = "" Then
            vcFrmLevel.vParentFrm.sForwardMessage("188", Me)
            Txt_PublicCleaning.Select()
            Return False
        End If
    ...
    Private Sub sSaveMain()
        ...
        If vMasterBlock = "I" Then
            ...
            vSqlCommand = " Insert Into HK_Cleaning_Public (  Company_Code,      Code,         TDate,   User_Code,            Remarks,                CleanType_Code,            Supervisor_Code,        Public_Code,                                                                       Points  ) " &
                         $"                     Values  ( {vCompanyCode}, {Txt_Code.Text}, {vDate}, {vUsrCode}, '{Trim(Txt_Remarks.Text)}',   {Txt_CleaningType.Value},  {Txt_Supervisor.Value}, {Txt_PublicCleaning.Value}, dbo.fn_Calc_Clean_Public_Points({Txt_PublicCleaning.Value}, {Txt_CleaningType.Value}, {vCompanyCode }) ) "

            sFillSqlStatmentArray(vSqlCommand)

        ElseIf vMasterBlock = "U" Then

            vSqlCommand = " Update HK_Cleaning_Public                               " &
                         $" Set   Public_Code       = '{Txt_PublicCleaning.Value}',        " &
                         $"       CleanType_Code  = '{Txt_CleaningType.Value}', " &
                         $"       Supervisor_Code = '{Txt_Supervisor.Value}',   " &
                         $"       User_Code       = '{vUsrCode}',               " &
                         $"       Remarks         = '{Trim(Txt_Remarks.Text)}', " &
                         $"       Points          =  dbo.fn_Calc_Clean_Public_Points({Txt_PublicCleaning.Value}, {Txt_CleaningType.Value}, {vCompanyCode })  " &
                         $" Where Code            = '{Txt_Code.Text}'           " &
                         $" And   Company_Code    = '{vCompanyCode}'            "
...
#Region " Form Level                                                                     "
    Private Sub Txt_SearchEmployee_EditorButtonClick(sender As Object, e As UltraWinEditors.EditorButtonEventArgs) Handles _
            Txt_LovEmployee.EditorButtonClick
    ...
    Handles Txt_Remarks.Enter, Txt_EmpDesc.Enter, Txt_Code.Enter, Txt_CleaningDate.Enter
    ...
    Private Sub sLoad_PublicPlaces()
        Try
            Dim vsqlCommand As New SqlCommand
            ...
            cControls.vSqlConn.Open()
            Dim vSqlReader As SqlDataReader = vsqlCommand.ExecuteReader
            Txt_PublicCleaning.Items.Clear()

            Do While vSqlReader.Read
                Txt_PublicCleaning.Items.Add(vSqlReader("Code"), vSqlReader("Desc" & vLang))
            Loop
            cControls.vSqlConn.Close()
            Txt_PublicCleaning.SelectedIndex = -1
    ...
    Private Sub sSaveDetails()
        ...
        For Each vRow In Grd_Details.Rows
            ...
            If vRow.Cells("DML").Value = "I" Then
                ...
                vSqlString = " Select IsNull(Max(Ser), 0) + 1 From HK_Cleaning_Public_Details " &
                             " Where  Cl_Code = " & Txt_Code.Text &
                             " And    Company_Code = " & vCompanyCode

                vGetSerial = cControls.fReturnValue(vSqlString, Me.Name) + vCounter

                vSqlString = " Insert Into HK_Cleaning_Public_Details (     Ser,       Company_Code,      Cl_Code,                  Emp_Code,                       Remarks          ) " &
            ...
            ElseIf vRow.Cells("DML").Value = "U" Then
                vSqlString = " Update HK_Cleaning_Public_Details                                  " &
...
#Region " Query                                                                          "
    Private Sub sQueryDetails()
        Try
            ...
            vsqlCommand.CommandText =
            " Select HK_Cleaning_Public_Details.Ser,                          " &
            "        HK_Cleaning_Public_Details.Remarks,                      " &
            "        HK_Cleaning_Public_Details.Emp_Code,                     " &
            "        Employees.DescA                                      " &
            " From   HK_Cleaning_Public_Details Inner Join Employees          " &
            " On     HK_Cleaning_Public_Details.Emp_Code = Employees.Code     " &
            " Where  HK_Cleaning_Public_Details.Cl_Code =     " & Txt_Code.Text &
            " And    HK_Cleaning_Public_Details.Company_Code = " & vCompanyCode &
            " Order  By Ser "
...
#Region " Query                                                                                 "
    Private Sub sQuerySummary()
        ...
        Try
            ...
            vSqlCommand.CommandText =
                                    " SELECT                                                                   " &
                                    "       HK_Cleaning_Public.Code AS Code,                                      " &
                                    "       HK_Public_Types.DescA AS Public_Place,                       " &
                                    "       HK_Cleaning_Types.DescA AS Cleaning_Type,                             " &
                                    "       Employees.DescA AS Supervisor,                                     " &
                                    "       HK_Cleaning_Public.TDate AS Cleaning_Date,                            " &
                                    "       HK_Cleaning_Public.Remarks AS Public_Remarks                          " &
                                    "                                                                          " &
                                    " FROM  HK_Cleaning_Public                                                    " &
                                    "                                                                          " &
                                    " LEFT  JOIN HK_Public_Types                                         " &
                                    ...
                                    " ON    HK_Cleaning_Public.CleanType_Code = HK_Cleaning_Types.Code        " &
                                    " AND   HK_Cleaning_Public.Company_Code = HK_Cleaning_Types.Company_Code  " &
                                    "                                                                          " &
                                    " LEFT  JOIN Employees                                                     " &
                                    " ON    HK_Cleaning_Public.Supervisor_Code = Employees.Code                   " &
                                    " AND   HK_Cleaning_Public.Company_Code = Employees.Company_Code              " &
                                    "                                                                          " &
                                   $" WHERE HK_Cleaning_Public.Company_Code = {vCompanyCode}                      " &
                                   $" AND   HK_Cleaning_Public.TDate BETWEEN {vFromDate} AND {vToDate_PlusOneDay} "
            ...
            Do While vSqlReader.Read
                ...
                'Public_Place
                If IsDBNull(vSqlReader("Public_Place")) = False Then
                    DTS_Summary.Rows(vRowCounter)("Public_Place") = vSqlReader("Public_Place")
                Else
                    DTS_Summary.Rows(vRowCounter)("Public_Place") = Nothing
                End If
                ...
                'Public_Remarks
                If IsDBNull(vSqlReader("Public_Remarks")) = False Then
                    DTS_Summary.Rows(vRowCounter)("Public_Remarks") = vSqlReader("Public_Remarks")
                Else
                    DTS_Summary.Rows(vRowCounter)("Public_Remarks") = Nothing
                End If
